...
public:
    ...
    calculateLightIntensity(std::vector<std::shared_ptr<Primitive>> scenePrimitives, Vector3 cameraPosition,
                            const Primitive &intersectedPrimitive, Intersection intersection) = 0;
    ...
    virtual bool isAccessible(Vector3 reflectionPoint, const Primitive &intersectedPrimitive,
                              std::vector<std::shared_ptr<Primitive>> scenePrimitives) = 0;
};
